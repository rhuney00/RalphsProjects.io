$(document).ready( function(){
	...
	function turn(turnNumber, players) {
		myVar = setInterval(function() {
		if((players == 2) || (players == 1 && turnNumber == 1)) {
			row = askRow(turnNumber)
			col = askColumn(turnNumber);
		}
		else if(players == 1 && turnNumber == 2) {
			var compMove = computerMove();
			row = compMove[0];
			col = compMove[1];
		}
		var goodMove = viableMove(row, col);
		if(goodMove == 1) { placeMarker(row, col, turnNumber);}
		if(turnNumber == 1 && goodMove == 1) {turnNumber = 2;}
		else if(turnNumber ==2 && goodMove == 1) {turnNumber = 1;}
